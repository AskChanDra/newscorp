...
function wpcorp_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'wpcorp' ),
 ...
 * @return void
 */
function wpcorp_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'wpcorp' ),
 ...
 * @return bool|string
 */
function wpcorp_the_excerpt( $trim_character_count = 0 ) {
 ...
 */
function wpcorp_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="wpcorp-read-more text-white" href="%1$s"><button class="mt-3 btn btn-info">%2$s</button></a>',
...
function wpcorp_pagination() {
	...
	printf( '<nav class="wpcorp-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
